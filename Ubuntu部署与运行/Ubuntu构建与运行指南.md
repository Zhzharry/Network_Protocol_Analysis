...
**答辩/展示时可用的功能型选项（克制、不花哨）**：`--preflight` 先输出运行前检查；程序结束时有「查询结果摘要」（含往返时延 RTT、RR 条数、IPv4 / CNAME 要点）。可附加 `--export-report report.md`、`--json-out session.json` 或 `--emit-json` 便于留档与脚本处理；不需要结尾摘要时加 `--no-summary`。详见 `python3 ... --help`。

**原始帧十六进制与文本报告**：`--hex` 在控制台附录打印**完整请求/应答以太网帧**的经典 hex+ASCII 转储；`--export-txt lab.txt` 生成**纯文本实验报告**（参数、RR 列表、格式化转储 + **连续无空格 hex** 便于检索）。可同时使用：`sudo python3 ... 8.8.8.8 www.baidu.com --iface eth0 --hex --export-txt dns_lab.txt`。
...
**`--type`（A / AAAA / CNAME / NS / MX / TXT）与类型码对照、多条可复制命令示例、运行后大致输出说明**，见同目录 **[DNS记录类型与命令行示例.md](./DNS记录类型与命令行示例.md)**（含 `python3 my_nslookup/my_nslookup.py 8.8.8.8 baidu.com --type NS` 等）。
