...
def print_executive_summary(
    ...
    *,
    ...
    """
    会话结束时的结构化结果摘要（功能向：RTT、记录数、解析要点），风格克制。
    """
...
def print_wire_hex_appendix(
    ...
    *,
    ...
    """在协议字段解析之后，附录：完整请求/应答帧的十六进制转储。

    不用 Panel 包裹正文，避免终端宽度下 Rich 对长行右侧截断；标题用 Rule 分隔。
    """
    from hexdump_fmt import format_hex_dump
